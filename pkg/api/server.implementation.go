...
func (s *ServerImplementation) WelcomeMessage(ctx echo.Context) error {
	return ctx.JSON(http.StatusOK, map[string]string{"message": "Welcome to Movie Catalogue"})
}

func (s *ServerImplementation) GetMovieByName(ctx echo.Context, name string) error {
	var movies []Movie
	tx := s.DB.Where("title LIKE ?", "%"+name+"%").Find(&movies)
	if tx.Error != nil {
...
func (s *ServerImplementation) DeleteMovieByName(ctx echo.Context, name string) error {
	s.Lock.Lock()
	defer s.Lock.Unlock()

	var movie Movie
	tx := s.DB.Where("title = ?", name).First(&movie)
	if tx.Error != nil {
		return ctx.JSON(http.StatusNotFound, "Movie not found")
	}

	tx = s.DB.Delete(&movie)
	if tx.Error != nil {
		return ctx.JSON(http.StatusInternalServerError, tx.Error)
	}

	return ctx.JSON(http.StatusOK, "Movie deleted")
}
